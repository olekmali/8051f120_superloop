...
static uint16_t kill_switch  = 0;

static uint16_t semaph_epoch = 1;
static uint16_t pwm_epoch    = 100;
static uint32_t kill_epoch   = 1000;
...
void Timer3_Init (uint32_t sysclock, uint32_t rate, uint32_t rate_pwm, uint32_t rate_semaph, uint16_t rate_killsw)
{
    ...
    kill_epoch   = rate / rate_killsw;  // how many interrupts max between PWM update before emergency power down
...
void Timer3_setPWMDuty(uint8_t new_rate)
{
    ...
    kill_switch = kill_epoch;           // restart dead man count down
